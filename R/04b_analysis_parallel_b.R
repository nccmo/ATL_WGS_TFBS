...
tst <- commandArgs(trailingOnly = TRUE)[5]
n_sample <- commandArgs(trailingOnly = TRUE)[6] %>% as.numeric()


dir_output_table <-
  str_glue("./result/table/{tst}/slop{slop}/wlen{window}")
dir_output_pdf <-
  str_glue("./result/pdf/{tst}/slop{slop}/wlen{window}")
...
df <-
  read_tsv(
    str_glue(
      "/home/kogu/analysis/ner_analysis/result_count/{tst}/wlen{window}/merged_ENCODE.tf.bound.union__rep{rep}b_{bin}.slop{slop}__{window}bp_noCDS_callable__count.txt"
    ),
...
dfv <-
  read_tsv(
    str_glue(
      "/home/kogu/analysis/ner_analysis/result_count/{tst}/wlen{window}/merged_ENCODE.tf.bound.union__rep{rep}b_{bin}.slop{slop}_v__{window}bp_noCDS_callable__count.txt"
    ),
    col_names = c("chrom", "start", "end", "name", "count")
  ) %>%
  mutate(bin_length = end - start)
...
dfp <-
  read_tsv(
    str_glue(
      "/home/kogu/analysis/ner_analysis/result_count/{tst}/wlen{window}/merged_ENCODE.tf.bound.union__rep{rep}b_{bin}.slop{slop}_peri__{window}bp_noCDS_callable__count.txt"
    ),
    col_names = c("chrom", "start", "end", "name", "count")
  ) %>%
  mutate(bin_length = end - start)
...
gg <- ggplot(dfj) +
  geom_jitter(aes(".Near", rate_by_tfbs_per_megabase)) +
  geom_jitter(aes("Distant", rate_by_tfbs_per_megabase_v),
              color = "red") +
  geom_jitter(aes("Periphery", rate_by_tfbs_per_megabase_p),
              color = "blue") +
...
df_stat <- 
  t.test(dfj$rate_by_tfbs_per_megabase,
       dfj$rate_by_tfbs_per_megabase_v,
       paired = T) %>%
  tidy() %>% 
  mutate(type = "near_vs_distant") %>% 
  bind_rows(df_stat)

df_stat <- 
  t.test(dfj$rate_by_tfbs_per_megabase,
       dfj$rate_by_tfbs_per_megabase_p,
       paired = T) %>%
  tidy() %>% 
  mutate(type = "near_vs_periphery") %>% 
  bind_rows(df_stat)

df_stat <- 
  t.test(dfj$rate_by_tfbs_per_megabase_v,
       dfj$rate_by_tfbs_per_megabase_p,
       paired = T) %>%
  tidy() %>% 
  mutate(type = "distant_vs_periphery") %>% 
  bind_rows(df_stat)

df_stat <- 
  wilcox.test(dfj$rate_by_tfbs_per_megabase,
            dfj$rate_by_tfbs_per_megabase_v,
            paired = T) %>% 
  tidy() %>% 
  mutate(type = "near_vs_distant") %>% 
...
df_stat <- 
  wilcox.test(dfj$rate_by_tfbs_per_megabase_v,
            dfj$rate_by_tfbs_per_megabase_p,
            paired = T) %>% 
  tidy() %>% 
  mutate(type = "distant_vs_periphery") %>% 
...
df_stat %>% 
  write_excel_csv(
    str_glue(
      "{dir_output_table}/03c_fulljoin_stats_{tst}_{window * 2 + 1}bp_rep{rep}b_{bin}.csv"
    )
  )
